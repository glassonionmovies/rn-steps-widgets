...
// Interim exercise planner with a stable I/O interface.
// You can later swap internals for GPT without changing callers.
...
export function generatePlan(input) {
  ...
  function blendReadiness(v){ const arr=[v.readiness,v.energy,v.sleepQuality].filter(x=>typeof x==='number'&&isFinite(x)); return arr.length? clamp(arr.reduce((a,b)=>a+b,0)/arr.length,0,1) : 0.8; }
  ...
  function deDuplicatePatterns(blocks){
  ...
  function smartName(split,goals,groups){
