...
function startOfWeek(ts) {
  const d = new Date(ts); const day = d.getDay(); d.setDate(d.getDate() - day);
    ...
    >
      ...
      <View style={{ position: 'absolute', left: AXIS_W, top: 0, right: 0, bottom: BOT_H }}>
        {axis.ticks.map((t, idx) => {
          ...
          return <View key={`g-${idx}`} style={{ position: 'absolute', left: 0, right: 0, top: y, height: 1, backgroundColor: '#E5E7EB' }} />;
        })}
      </View>
      {/* y labels */}
      <View style={{ position: 'absolute', left: 0, top: 0, width: AXIS_W, bottom: BOT_H }}>
        {axis.ticks.map((t, idx) => {
          const y = innerH - (innerH * t) / axis.max;
          return <Text key={`yl-${idx}`} style={{ position: 'absolute', right: 4, top: y - 7, fontSize: 10, color: '#6B7280' }}>{fmtK(t)}</Text>;
        })}
      </View>
      {/* bars */}
        ...
        {Array.from({ length: n }).map((_, i) => {
          ...
          return (
            <View key={`bar-${i}`} style={{ width: barW, marginRight: i === n - 1 ? 0 : gap, alignItems: 'center' }}>
              <View style={{ width: barW, height: h, borderTopLeftRadius: 6, borderTopRightRadius: 6, backgroundColor: color }} />
            </View>
          );
        })}
      </View>
...
export default function VolumeReportCard({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // how many weekly buckets?
      const weeks = Math.max(4, Math.ceil(rangeDays / 7));
      ...
      (all || [])
        ...
        .forEach((w) => {
          ...
          const vol = (w.blocks || []).reduce((acc, b) => {
            ...
            const blockVol = (b.sets || []).reduce((s, x) => s + (Number(x.weight) || 0) * (Number(x.reps) || 0), 0);
            return acc + blockVol;
          }, 0);
      ...
      if (active) setSeries({ labels: buckets.map(b => b.label), values: buckets.map(b => b.value) });
    })();
    return () => { active = false; };
  }, [group, rangeDays]);
  ...
  return (
    <Card style={{ padding: spacing(2) }}>
      <View style={{ flexDirection: 'row', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: spacing(1) }}>
        ...
        <Text style={{ color: palette.sub, fontSize: 12 }}>Last {rangeDays} days</Text>
      </View>
